...
export function TechnologyManager({ initialTechnologies }: TechnologyManagerProps) {
  ...
  const handleToggleSelectAllPage = () => {
    const pageIds = paginatedTechnologies.map((t) => t.id);
  ...
  const handleOpenDelete = (tech: AdminTechnologyWithCount) => {
    if (tech.resourceCount > 0) {
      toast.error(
        `Teknologi "${tech.name}" tidak dapat dihapus karena masih digunakan oleh ${tech.resourceCount} komponen.`
      );
      return;
    }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "Gagal menghapus teknologi.";
      toast.error(msg);
    } finally {
      setIsDeleting(false);
  ...
  const handleConfirmBulkDelete = async () => {
    ...
    // Filter out technologies that are currently attached to resources
    const inUse = technologies.filter((t) => selectedIds.has(t.id) && t.resourceCount > 0);
    if (inUse.length > 0) {
      toast.error(
        `${inUse.length} teknologi tidak dapat dihapus karena masih digunakan oleh komponen aktif.`
      );
  ...
  const isPagePartiallySelected =
    paginatedTechnologies.some((t) => selectedIds.has(t.id)) && !isPageAllSelected;
  ...
  const totalUsedCount = technologies.reduce((sum, t) => sum + (t.resourceCount > 0 ? 1 : 0), 0);
  ...
  return (
    <div ref={tableContainerRef} className="space-y-6 scroll-mt-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-h2 text-[#272343]">Kelola Teknologi</h1>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs border-b border-[#BAE8E8]/60 pb-3">
        ...
        <span className="px-2.5 py-0.5 rounded-full bg-[#E3F6F5] border border-[#BAE8E8] text-[#272343] font-mono text-[11px]">
          <strong>{technologies.length}</strong> Total
        </span>
        ...
        <span className="px-2.5 py-0.5 rounded-full bg-amber-50 border border-amber-200 text-amber-800 font-mono text-[11px]">
          <strong>{technologies.length - totalUsedCount}</strong> Kosong
        </span>
        {filteredTechnologies.length !== technologies.length && (
          <span className="px-2.5 py-0.5 rounded-full bg-blue-50 border border-blue-200 text-blue-800 font-mono text-[11px]">
            Menampilkan <strong>{filteredTechnologies.length}</strong> hasil filter
          </span>
        )}
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="relative flex-1 max-w-md">
          ...
          <Input
            placeholder="Cari teknologi berdasarkan nama, slug, atau deskripsi..."
            value={searchQuery}
      ...
      {selectedIds.size > 0 && (
        <div className="p-3 rounded-xl bg-[#272343] text-white shadow-soft-lg flex flex-wrap items-center justify-between gap-3 animate-in fade-in slide-in-from-top-2 duration-150">
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="font-semibold">Teknologi terpilih</span>

            {selectedIds.size < filteredTechnologies.length && (
              <button
                type="button"
                onClick={handleSelectAllFiltered}
                className="ml-2 text-xs text-[#BAE8E8] hover:text-white underline underline-offset-2"
              >
                Pilih semua ({filteredTechnologies.length})
              </button>
            )}
          </div>
        ...
        <CardContent className="p-0">
          {paginatedTechnologies.length > 0 ? (
              ...
              <table className="w-full text-left text-xs">
                <thead className="bg-[#E3F6F5]/50 border-b border-[#BAE8E8] text-[#2D334A] font-semibold">
                  <tr>
                    ...
                    <th className="py-3 px-4 font-semibold text-[#272343]">Teknologi</th>
                        ...
                        <td className="py-3.5 px-4 font-bold text-[#272343]">
                          <div className="flex items-center gap-2">
                            {item.icon ? (
                              ...
                              <Cpu className="h-4 w-4 text-[#0D6E6E] shrink-0" />
                            )}
                            <span>{item.name}</span>
                          </div>
                        </td>
            ...
            <div className="p-12 text-center text-xs text-[#2D334A]/60 space-y-2">
              <Cpu className="h-8 w-8 mx-auto text-[#BAE8E8]" />
              <p className="font-semibold text-sm text-[#272343]">Tidak ada teknologi ditemukan</p>
              <p className="text-[11px]">
                {searchQuery
                  ? `Tidak ada teknologi yang cocok dengan kata kunci "${searchQuery}".`
                  : "Belum ada teknologi yang terdaftar."}
              </p>
            </div>
          )}
        </CardContent>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 border-t border-[#BAE8E8]/60 bg-[#FBFDFD]">
            ...
            <div className="flex flex-wrap items-center gap-3 text-[#2D334A]/80 w-full sm:w-auto justify-between sm:justify-start">
              <span className="text-xs">
                ...
                <strong className="text-[#272343] font-semibold">{totalItems}</strong> teknologi
              </span>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between border-b border-[#BAE8E8]/60 pb-3">
              <div className="flex items-center gap-2">
                <Cpu className="h-5 w-5 text-[#272343]" />
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs">
              <div className="space-y-1.5">
                ...
                <Input
                  required
                  placeholder="contoh: React, Tailwind CSS, Next.js, Framer Motion"
                  value={name}
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  rows={3}
                  placeholder="Deskripsi singkat mengenai teknologi atau framework ini..."
                  value={description}
      ...
      <DeleteConfirmDialog
        ...
        title="Hapus Teknologi Terpilih"
        description={`Apakah Anda yakin ingin menghapus ${selectedIds.size} teknologi yang dipilih? Tindakan ini tidak dapat dibatalkan.`}
